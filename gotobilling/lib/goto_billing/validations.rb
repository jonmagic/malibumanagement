module GotoBilling
  ...
  #   rescue GotoBilling::RecordInvalid => invalid
  #     puts invalid.record.errors
  #   end
  class RecordInvalid < GotoBillingError #:nodoc:
    attr_reader :record
  ...
  class Errors
    ...
    #
    #   class Company < GotoBilling::Base
  ...
  #
  # Please do have a look at GotoBilling::Validations::ClassMethods for a higher level of validations.
  module Validations
    ...
    module ClassMethods
      ...
      def validates_confirmation_of(*attr_names)
        configuration = { :message => GotoBilling::Errors.default_error_messages[:confirmation], :on => :save }
      ...
      def validates_acceptance_of(*attr_names)
        configuration = { :message => GotoBilling::Errors.default_error_messages[:accepted], :on => :save, :allow_nil => true, :accept => "1" }
        configuration.update(attr_names.pop) if attr_names.last.is_a?(Hash)
      ...
      def validates_presence_of(*attr_names)
        configuration = { :message => GotoBilling::Errors.default_error_messages[:blank], :on => :save }
        configuration.update(attr_names.pop) if attr_names.last.is_a?(Hash)

        if(configuration.has_key?(:either) && configuration.has_key?(:or))
          errors = []
          group1 = configuration[:either]
          group2 = configuration[:or]

          send(validation_method(configuration[:on])) do |record|
            group1_pass = true
            unless configuration[:if] and not evaluate_condition(configuration[:if], record)
              group1.each do |attr_name|
                value = record.respond_to?(attr_name.to_s) ? record.send(attr_name.to_s) : record[attr_name.to_s]
                if value.blank?
                  group1_pass = false
                elsif group1_pass
                  group1_pass = true
            ...
            unless configuration[:if] and not evaluate_condition(configuration[:if], record)
              ...
              record.errors.add_to_base("Must have EITHER #{group1.to_sentence} OR #{group2.to_sentence}") unless group1_pass ^ group2_pass
            end
          end
        else
      ...
      def validates_length_of(*attrs)
        ...
        options = {
          :too_long     => GotoBilling::Errors.default_error_messages[:too_long],
          :too_short    => GotoBilling::Errors.default_error_messages[:too_short],
          :wrong_length => GotoBilling::Errors.default_error_messages[:wrong_length]
      ...
      #
      #   class TeacherSchedule < GotoBilling::Base
      ...
      def validates_uniqueness_of(*attr_names)
        configuration = { :message => GotoBilling::Errors.default_error_messages[:taken], :case_sensitive => true }
        configuration.update(attr_names.pop) if attr_names.last.is_a?(Hash)
      ...
      def validates_format_of(*attr_names)
        configuration = { :message => GotoBilling::Errors.default_error_messages[:invalid], :on => :save, :with => nil }
        configuration.update(attr_names.pop) if attr_names.last.is_a?(Hash)
      ...
      def validates_inclusion_of(*attr_names)
        configuration = { :message => GotoBilling::Errors.default_error_messages[:inclusion], :on => :save }
        configuration.update(attr_names.pop) if attr_names.last.is_a?(Hash)
      ...
      def validates_exclusion_of(*attr_names)
        configuration = { :message => GotoBilling::Errors.default_error_messages[:exclusion], :on => :save }
        configuration.update(attr_names.pop) if attr_names.last.is_a?(Hash)
      ...
      def validates_associated(*attr_names)
        configuration = { :message => GotoBilling::Errors.default_error_messages[:invalid], :on => :save }
        configuration.update(attr_names.pop) if attr_names.last.is_a?(Hash)
      ...
      def validates_numericality_of(*attr_names)
        configuration = { :message => GotoBilling::Errors.default_error_messages[:not_a_number], :on => :save,
