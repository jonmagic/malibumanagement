...
require 'cgi'
require 'benchmark'
require 'goto_billing/connection/response'
...
module GotoBilling
  ...
  class Connection
    ...
    def initialize(site, merchant_id, merchant_pin, ip_address, format = ActiveResource::Formats[:xml])
      ...
      format.decode(
        request(:post,
          ...
          data.update(
            ...
            'merchant_pin' => @merchant_pin,
            'ip_address' => @ip_address
          ).map {|k,v| "#{urlencode(k.to_s)}=#{urlencode(v.to_s)}" }.join('&'),
