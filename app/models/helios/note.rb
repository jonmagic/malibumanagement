class Helios::Note < ActiveRecord::Base
  ...
  def self.create_on_master(attrs)
    rec = self.master[self.master.keys[0]].create(attrs.merge(:Last_Mdt => Time.now - 5.hours))
    rec.id
  end
  ...
  def update_on_master(attrs)
    self.class.update_on_master({:Client_no => self.Client_no, :OTNum => self.OTNum}.merge(attrs))
  end
