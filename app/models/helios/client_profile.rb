...
# Commented because we shouldn't need these...
  ...
  def has_prepaid_membership?(date=nil)
    date ||= Time.now
    sql = case ::RAILS_ENV
    when 'development'
    ...
    mem_trans = Helios::Transact.find(:all, :conditions => [sql, '1', '2', self.id, date-47088000])

    lasting = {
      'VY'  => date-36720000, # 425 days
      'VY+' => date-47088000, # 545 days
      'V1M' => date-2592000,  # 30 days
      'V1W' => date-604800    # 7 days
    }
