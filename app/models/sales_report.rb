...
#These are actually not necessary!!
  has_many :stores, :finder_sql => 'SELECT stores.* FROM stores,form_instances fi WHERE fi.form_data_id=#{id} AND fi.form_data_type="SalesReport" AND stores.id=fi.store_id'
  has_many :form_types, :finder_sql => 'SELECT form_types.* FROM form_types,form_instances fi WHERE fi.form_data_id=#{id} AND fi.form_data_type="SalesReport" AND form_types.id=fi.form_type_id'
  has_many :notes, :finder_sql => 'SELECT notes.* FROM notes,form_instances fi WHERE fi.form_data_id=#{id} AND fi.form_data_type="SalesReport" AND AND notes.form_instance_id=fi.id'
