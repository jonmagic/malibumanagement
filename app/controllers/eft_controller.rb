...
# one-try each time, but ajax keeps retry-ing until it is done
  ...
  def submit_payments
    restrict('allow only admins') or begin
      return(render(:json => {:error => "Batch has not been locked!"}.to_json)) if !@batch.locked && params[:incoming_bucket].blank?
      ...
      Store.find(:all).each do |store|
        ...
        store.dcas.incoming_bucket = params[:incoming_bucket] if params[:incoming_bucket]

        if store.dcas.incoming_bucket != DCAS::DEFAULT_INCOMING_BUCKET
          logger.info "Incoming Bucket set manually: #{store.dcas.incoming_bucket}"
        elsif !@batch.locked
          return(render(:json => result.merge(:error => "Batch was unlocked in the middle of submitting!").to_json)) unless @batch.locked || params[:incoming_bucket]
        end
        ...
        begin # ACH batch submit
          if @batch.submit_locked?(ach_batch.filename)
            result[ach_batch.filename] = 'Skipped.'
          elsif ach_batch.payments.empty?
            result[ach_batch.filename] = 'No payments.'
          else
            result[ach_batch.filename] = store.dcas.submit_batch!(ach_batch, @batch) ? 'Uploaded.' : 'Failed.'
          end
        end
        begin # CC batch submit
          if @batch.submit_locked?(cc_batch.filename)
            result[cc_batch.filename] = 'Skipped.'
          elsif cc_batch.payments.empty?
            result[cc_batch.filename] = 'No payments.'
          else
