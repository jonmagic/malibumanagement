...
#This is for submitting edits. This is an ajax-specific function, normally auto-save like gmail but also via a button (like gmail).
  def update
    restrict('allow only store users') or begin
      ...
      if !params[:form_instance].nil? && !params[:form_instance][:assigned_to].blank? && @form.user_id != params[:form_instance][:assigned_to] && @form.assigned_to != params[:form_instance][:assigned_to]
