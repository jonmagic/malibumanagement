    ...
    module Paranoid
      ...
      module InstanceMethods #:nodoc:
        ...
        module ClassMethods
          ...
          def add_paranoid_sql_conditions(sql)
            conditions = (sql =~ /(.* WHERE )(.*)/)
            sql.sub!(/^(.*WHERE)(.*?)( GROUP.*)/i, "#{$1}" + self.replace_named_bind_variables("(#{table_name}.#{deleted_attribute} IS NULL OR #{table_name}.#{deleted_attribute} > :datetime)", {:datetime => Time.now}) + " AND (#{$2})")
            sql
          end
        ...
        def deleted?
          !self.deleted_at.nil? && self.deleted_at < Time.now
        end
